...
func main() {
	...
	if err != nil {
		fmt.Println("❌ Failed to init Cloudinary:", err)
		return
	}

	url, err := cld.UploadImage("https://s3-api.fpt.vn/fptvn-storage/2025-09-04/1756983257_thumbdragonball.jpg")
	if url == "" {
		fmt.Println("❌ Upload failed or returned empty URL")
	} else {
		fmt.Println("✅ Uploaded successfully:", url)
	}
}
